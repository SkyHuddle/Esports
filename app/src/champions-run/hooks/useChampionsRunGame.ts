...
import { simulateChampionsRun } from '../engine/simulation';
import {
  getDailyConstraint,
  getDateKey,
  estimatePercentile,
...
export function useChampionsRunGame() {
  ...
  const finalizeRun = useCallback(
    (finalPicks: DraftPick[]) => {
      ...
      const sim = simulateChampionsRun(finalPicks, { seed });
      setResult(sim);
      setPhase('result');

      recordAttempt(sim.championsWon, sim.perfectRun, sim.rosterScore, mode === 'daily');

      if (mode === 'daily') {
        const percentile = estimatePercentile(sim.rosterScore, sim.championsWon, sim.perfectRun);
        setDailyPercentile(percentile);
        saveDailyResult({
          date: dateKey,
          score: sim.rosterScore,
          championsWon: sim.championsWon,
          perfectRun: sim.perfectRun,
          record: sim.runSummary.record,
          headline: sim.runSummary.headline,
          percentile,
        });
  ...
  const selectPlayer = useCallback(
    (player: ValorantPlayer, naturalRole: RosterSlot) => {
