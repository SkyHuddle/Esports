...
const STATS_KEY = 'champions-run-stats';
const DAILY_KEY = 'champions-run-daily';
const HISTORY_KEY = 'champions-run-history';
...
export function loadDailyResult(dateKey: string): DailyRunResult | null {
  try {
    ...
    return {
      date: row.date,
      score: row.score ?? 0,
      championsWon: row.championsWon ?? false,
